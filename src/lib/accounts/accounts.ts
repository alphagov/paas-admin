...
export interface IAccountsUsersResponse {
  readonly users: readonly IAccountsUserResponse[]
}
...
export class AccountsClient {
  ...
  public async getPendingDocumentsForUserUUID (
    uuid: string
  ): Promise<readonly IUserDocument[]> {
    const response = await this.request({
      url: `/users/${uuid}/documents`
    })
    const data: readonly IUserDocumentResponse[] = response.data
  ...
  ): Promise<readonly IAccountsUser[]> {
    const response = await this.request({
      method: 'get',
