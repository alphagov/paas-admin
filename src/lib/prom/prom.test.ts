...
describe('lib/prom test suite', () => {
  ...
  it('should throw error when prometheus responds with 404', async () => {
    nockPrometheus
      .get(/api.v1.query\??/).reply(404, {error: 'not found'});

    const client = new PromClient(config.apiEndpoint, config.username, config.password, config.logger);
    await expect(client.getValue('http_response_2xx', new Date())).rejects.toThrowError(/failed with status 404/);
  });

  it('should throw error when prometheus responds with 404 no body', async () => {
    nockPrometheus
      .get(/api.v1.query\??/).reply(404);
  ...
  it('should getValue successfully', async () => {
    nockPrometheus
      .get(/api.v1.query\??/).reply(200, {
        status: 'success',
        data: {
          result: [{
            value: [
              moment().toDate().getTime() / 1000,
  ...
  it('should fail to getSeries when invalid query has been provided', async () => {
    nockPrometheus
      .get(/api.v1.query_range\??/).reply(200, {
        status: 'success',
        data: {
          result: [],
        },
