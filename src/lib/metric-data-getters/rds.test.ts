...
describe('RDS', () => {
  describe('getRdsDbInstanceIdentifier', () => {
    it('should prepend rdsbroker- to RDS service guids', () => {
      ...
      expect(dg.getRdsDbInstanceIdentifier('some-guid')).toBe('rdsbroker-some-guid');
      expect(dg.getRdsDbInstanceIdentifier('some-other-guid')).toBe('rdsbroker-some-other-guid');
    });
  });
});
