...
export default class CloudFoundryClient {
  ...
  constructor (config: IClientConfig) {
    ...
    this.clientCredentials = (config.clientCredentials != null) || {
      clientID: '',
      clientSecret: ''
    }
  ...
  public async request (
    ...
    return await request(this.apiEndpoint, method, url, this.logger, {
      data,
  ...
  public async allV3Resources<T>(
    ...
    const data: readonly T[] = response.data.resources
    ...
    const newData: readonly T[] = await this.allV3Resources(newResponse)
  ...
  public async organizations (): Promise<readonly cf.IOrganization[]> {
    const response = await this.request('get', '/v2/organizations')

    return await this.allResources(response)
  }

  public async v3Organizations (): Promise<
  readonly cf.IV3OrganizationResource[]
  > {
  ...
  }): Promise<readonly cf.IOrganizationQuota[]> {
    let query = ''
  ...
  public async organizationQuotas (): Promise<readonly cf.IV3OrganizationQuota[]> {
    const response = await this.request('get', '/v3/organization_quotas')

    return await this.allV3Resources(response)
  }

  public async applyOrganizationQuota (quotaGUID: string, ...organizationGUIDs: readonly string[]): Promise<object> {
  ...
  public async spaces (): Promise<readonly cf.ISpace[]> {
  ...
  public async applications (
    spaceGUID: string
  ): Promise<readonly cf.IApplication[]> {
  ...
  public async spaceServices (
    spaceGUID: string
  ): Promise<readonly cf.IServiceInstance[]> {
    const response = await this.request(
      'get',
  ...
  public async servicePlans (serviceGUID: string): Promise<readonly cf.IServicePlan[]> {
  ...
  public async usersForOrganization (
    organizationGUID: string
  ): Promise<readonly cf.IOrganizationUserRoles[]> {
    const response = await this.request(
      'get',
  ...
  public async usersForSpace (
    spaceGUID: string
  ): Promise<readonly cf.ISpaceUserRoles[]> {
    const response = await this.request(
      'get',
  ...
  public async userServices (
    spaceGUID: string
  ): Promise<readonly cf.IUserServices[]> {
    const response = await this.request(
      'get',
  ...
  public async stacks (): Promise<readonly cf.IStack[]> {
  ...
  public async auditEvents (
    ...
    spaceGUIDs?: readonly string[],
    orgGUIDs?: readonly string[]
    ...
    const resp = await this.request(
      ...
      /* params */ {
        order_by: '-updated_at',
        organization_guids: (orgGUIDs != null) ? orgGUIDs.join(',') : undefined,
        page,
        per_page: 25,
        space_guids: (spaceGUIDs != null) ? spaceGUIDs.join(',') : undefined,
        target_guids: (targetGUIDs != null) ? targetGUIDs.join(',') : undefined
      }
    )
  ...
  public async userRoles (userGUID: string): Promise<readonly cf.IRole[]> {
    const resp = await this.request(
      'get',
      '/v3/roles',
      /* data */ undefined,
    ...
    return await this.allV3Resources(resp)
  }
}
