...
describe(StatementsPage, () => {
  ...
  it('should parse statements page', () => {
    const markup = shallow(<StatementsPage
    ...
    expect($('th').text()).toContain('Total cost for January in space-name space with service-name services');
  ...
  it('should parse statements page and notify tenant that there\'s no information for this month', () => {
    ...
    />);
    const $ = cheerio.load(markup.html());
    expect($('.paas-table-notification').text()).toEqual('There is no record of any usage for that period.');
  });
});
