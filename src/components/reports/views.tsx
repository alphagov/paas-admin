...
import { IBillableByOrganisationAndService, IBillableByOrganisationAndSpaceAndService, IBillableByService } from './controllers';

interface IOrganizationsReportProperties {
  readonly organizations: readonly IV3OrganizationResource[];
  readonly trialOrgs: readonly IV3OrganizationResource[];
  readonly billableOrgs: readonly IV3OrganizationResource[];
...
interface ICostReportProperties {
  ...
  readonly orgCostRecords: readonly IOrgCostRecord[];
  readonly quotaCostRecords: readonly IQuotaCostRecord[];
}

interface ICostByServiceReport {
  readonly date: string;
  readonly billablesByService: readonly IBillableByService[];
  readonly billablesByOrganisationAndService: readonly IBillableByOrganisationAndService[];
  readonly billablesByOrganisationAndSpaceAndService: readonly IBillableByOrganisationAndSpaceAndService[];
}
...
export function OrganizationsReport(props: IOrganizationsReportProperties): ReactElement {
  return(<>
    ...
    <p className="govuk-body">
      ...
      {' '}
      {props.trialOrgs.length}
      {' '}
      trial {props.trialOrgs.length === 1 ? 'organisation' : 'organisations'}.
    </p>
      ...
      <tbody className="govuk-table__body">
       {props.trialOrgs.map(organization => (
         ...
         <td className="govuk-table__cell">
           <a href={props.linkTo('admin.organizations.view', {organizationGUID: organization.guid})} className="govuk-link">
             {organization.name}
           </a>
         </td>
         <td className="govuk-table__cell">
           {organization.metadata.annotations.owner === undefined ? 'Unknown' : organization.metadata.annotations.owner}
         </td>
         <td className="govuk-table__cell">
           {props.orgQuotaMapping[organization.relationships.quota.data.guid].entity.name}
         </td>
         <td className="govuk-table__cell">
           {moment(organization.created_at).format(DATE)}
         </td>
         <td className="govuk-table__cell">
           {new Date() > props.orgTrialExpirys[organization.guid] ? 'Expired' : 'Expires'}
           {' '}
    ...
    <p className="govuk-body">
      ...
      {' '}
      {props.billableOrgs.length}
      {' '}
      billable {props.billableOrgs.length === 1 ? 'organisation' : 'organisations'}.
    </p>
        ...
        <tr key={organization.guid} className="govuk-table__row">
          ...
          <td className="govuk-table__cell">
            {organization.metadata.annotations.owner === undefined ?
              'Unknown' : organization.metadata.annotations.owner}
          </td>
          <td className="govuk-table__cell">
            {props.orgQuotaMapping[organization.relationships.quota.data.guid].entity.name}
          </td>
...
export function CostReport(props: ICostReportProperties): ReactElement {
  return(<>
    ...
    <table className="govuk-table">
      <thead className="govuk-table__head">
        <tr className="govuk-table__row">
          ...
          <th className="govuk-table__header govuk-table__header--numeric" scope="col">
            Including VAT
          </th>
          <th className="govuk-table__header govuk-table__header--numeric" scope="col">
            Excluding VAT
    ...
    <h1 className="govuk-heading-l">
      Billables by quota for {props.date}
    </h1>
...
export function CostByServiceReport(props: ICostByServiceReport): ReactElement {
  return(<>
    <h1 className="govuk-heading-l">
      Billables by service for {props.date}
    </h1>
