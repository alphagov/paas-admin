...
export function Breadcrumbs(props: IBreadcrumbsProperties): ReactElement {
  const items = props.items.map((item, index) => (<li key={index} className="govuk-breadcrumbs__list-item"
    aria-current={index + 1 === props.items.length ? 'page' : undefined}>
    {item.href ? <a className="govuk-breadcrumbs__link" href={item.href}>{item.text}</a> : item.text}
  </li>));
