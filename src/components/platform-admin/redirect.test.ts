...
describe('redrect', () => {
  describe('when not a platform admin', () => {
    ...
    it('should return an error', async () => {
      await expect(redirectToPage(ctx, {}, {})).rejects.toThrow(
  ...
  describe('when a platform admin', () => {
    ...
    describe('when the action is find-user', () => {
      ...
      it('should redirect to the users page', async () => {
        const response = await redirectToPage(
          ctx,
          {},
          {
            action: 'find-user',
    ...
    describe('when the action is view-costs', () => {
      ...
      ['cost', 'cost-by-service', 'pmo-org-spend-csv', 'visualisation'].forEach(
        format => {
          it(`should redirect to the ${format} report`, async () => {
            ...
            expect(response.redirect).toEqual(
              `__LINKED_TO__admin.reports.${format}`
            )
          })
        }
      )
