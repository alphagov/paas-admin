...
import { IServiceInstance, IServicePlan, IService } from '../../lib/cf/types';
import { ServicePage, ServiceTab } from './views';

describe(ServiceTab, () => {
  it('should produce service tab', () => {
    const service = {metadata: {guid: 'SERVICE_GUID'}, entity: {name: 'service-name'}} as unknown as IServiceInstance;
    const markup = shallow(<ServiceTab
      service={service}
      linkTo={route => `__LINKS_TO__${route}`}
      routePartOf={route => route === 'admin.organizations.spaces.services.view'}
    ...
    expect($('ul li:last-of-type a').prop('href'))
      .toEqual('__LINKS_TO__admin.organizations.spaces.services.events.view');
...
describe(ServicePage, () => {
  it('should not display default values', () => {
    const service = {
      ...
      entity: { last_operation: { state: 'success' }, name: 'service-name', tags: [] },
    } as unknown as IServiceInstance;
    const markup = shallow(<ServicePage
      service={{
        ...service,
        service_plan: { entity: { name: 'service-plan-name' } } as unknown as IServicePlan,
        service: { entity: { label: 'service-label' } } as unknown as IService,
      }}
      linkTo={route => `__LINKS_TO__${route}`}
      routePartOf={route => route === 'admin.organizations.spaces.services.view'}
      organizationGUID="ORG_GUID"
