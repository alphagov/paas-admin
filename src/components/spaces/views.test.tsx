...
import { IAuditEvent, IAuditEventActorTarget, ISpace } from '../../lib/cf/types';
import {
  ApplicationsPage,
  BackingServicePage,
  EventsPage,
...
describe(EventsPage, () => {
  ...
  const space = {metadata: {guid: 'SPACE_GUID'}, entity: {name: 'SPACE_NAME'}} as unknown as ISpace;

  it('should corretly print multiple events on the page', () => {
    const markup = shallow(<EventsPage
      ...
      events={[
        event,
        {
          ...event,
          type: 'tester.testing',
          actor: {...event.actor, guid: 'ACCOUNTS_USER_GUID_2', name: 'Charlie Chaplin'},
          target: {guid: 'ACCOUNTS_USER_GUID_3'} as unknown as IAuditEventActorTarget,
        },
        {
          ...event,
          type: 'tester.testing',
          actor: {...event.actor, guid: 'ACCOUNTS_USER_GUID_3', name: undefined},
          target: {guid: 'ACCOUNTS_USER_GUID_1', name: 'Jeff Jefferson'} as unknown as IAuditEventActorTarget,
        },
      ]}
    ...
    expect($('table tbody .actor').text()).toContain(actorEmails.ACCOUNTS_USER_GUID_1);
    ...
    expect($('table tbody .actor').text()).not.toContain('ACCOUNTS_USER_GUID_2');
    expect($('table tbody .description').text()).toContain('tester.testing');
    expect($('table tbody .target code').text()).toContain('ACCOUNTS_USER_GUID_3');

    expect($('table tbody .actor code').text()).toContain('ACCOUNTS_USER_GUID_3');
    expect($('table tbody .target').text()).toContain(actorEmails.ACCOUNTS_USER_GUID_1);
  });
});
...
describe(ApplicationsPage, () => {
  ...
  const application = {
    ...
    summary: { running_instances: 1, instances: 2, memory: GIBIBYTE, disk_quota: GIBIBYTE, state: 'running' },
    urls: ['test.example.com'],
  } as unknown as IEnchancedApplication;

  it('should print correct phrasing when single service listed', () => {
    const markup = shallow(<ApplicationsPage
      linkTo={route => `__LINKS_TO__${route}`}
      routePartOf={route => route === 'admin.organizations.spaces.applications.events.view'}
      organizationGUID="ORG_GUID"
      space={space}
...
describe(BackingServicePage, () => {
  ...
  ] as unknown as ReadonlyArray<IEnchancedServiceInstance | IStripedUserServices>;

  it('should correctly print the backing service page', () => {
    const markup = shallow(<BackingServicePage
      linkTo={route => `__LINKS_TO__${route}`}
      routePartOf={route => route === 'admin.organizations.spaces.applications.events.view'}
      organizationGUID="ORG_GUID"
      services={services}
    ...
    expect($('table tbody .label').text()).toContain(services[0].definition.entity.label);
    expect($('table tbody .label').text()).toContain('User Provided Service');
    expect($('table tbody .plan').text()).toContain(services[0].plan.entity.name);
    expect($('table tbody .plan').text()).toContain('N/A');
    expect($('table tbody .status').text()).toContain(services[0].entity.last_operation.state);
...
describe(SpacesPage, () => {
  const organization = {
    ...
    quota: { entity: { name: 'default', memory_limit: 5 * GIBIBYTE / MEBIBYTE } },
  ...
  it('should correctly render the spaces page', () => {
    const markup = shallow(<SpacesPage
      linkTo={route => `__LINKS_TO__${route}`}
      isAdmin={false} isManager={false} isBillingManager={false}
    ...
    expect($('p').text()).toContain('Using 1.00 GiB of memory out of a maximum of 5.00 GiB.');
    expect($('a').text()).toContain('View  team members');
    expect($('span').text()).toContain('Trial');
    expect($('p').text()).toContain('Trial organisations have limited access to backing services');
    expect($('p').text()).toContain(`There are 2 spaces in ${organization.entity.name}.`);
  ...
  it('should correctly render the spaces page with single space', () => {
    const markup = shallow(<SpacesPage
      linkTo={route => `__LINKS_TO__${route}`}
      isAdmin={false} isManager={true} isBillingManager={false}
      organization={organization}
      spaces={[space]}
      users={[null]}
    />);
    ...
    expect($('p').text()).toContain(`There is 1 space in ${organization.entity.name}.`);
  });
});
