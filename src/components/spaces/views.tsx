...
import { Details, TargetedEvent, TargetedEventListItem, Totals } from '../events';
...
interface IBackingServicePageProperties {
  ...
  readonly routePartOf: RouteActiveChecker;
  readonly space: ISpace;
  readonly services: ReadonlyArray<IEnchancedServiceInstance | IStripedUserServices>;
}
...
export function SpacesPage(props: ISpacesPageProperties): ReactElement {
  return (<>
    ...
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-one-third">
        <div className="org-summary-stat">
          <h2 className="org-summary-stat__heading">
            Quota usage
          </h2>
        ...
        <p className="org-summary-stat__context govuk-body">
          Using {help.bytesToHuman(props.organization.memory_allocated * MEBIBYTE)}
          {' '}
          of memory out of a maximum of
          {' '}
          {help.bytesToHuman(props.organization.quota.entity.memory_limit * MEBIBYTE)}.
        </p>

        <p className="govuk-body">
          <a href="https://docs.cloud.service.gov.uk/managing_apps.html#quotas" className="govuk-link">
      ...
      <div className="govuk-grid-column-one-third">
        ...
        <p className="govuk-body">
          <a href={props.linkTo('admin.organizations.users', {
            ...
            View {props.isAdmin || props.isManager ? 'and manage' : ''} team members
          </a>
        </p>
      </div>
      ...
      <div className="govuk-grid-column-one-third">
        <div className="org-summary-stat">
          ...
          <span className="org-summary-stat__figure">
            {props.organization.quota.entity.name === 'default' ? 'Trial' : 'Billable'}
          </span>
        </div>
        ...
        {help.conditionallyDisplay(props.isAdmin || props.isManager || props.isBillingManager,
          <p className="govuk-body">
            <a href={props.linkTo('admin.statement.dispatcher', {
              organizationGUID: props.organization.metadata.guid,
            })} className="govuk-link">
    ...
    <p className="govuk-body">
      There {props.spaces.length === 1 ? 'is' : 'are'}
      {' '}
      {props.spaces.length} {props.spaces.length === 1 ? 'space' : 'spaces'}
      {' '}
      in {props.organization.entity.name}.
    </p>
    ...
    <details className="govuk-details" role="group">
      ...
      <div className="govuk-details__text" aria-hidden="true">
        <p>
          Each organisation contains one or more spaces, which are used to organise app development, deployment, and
          maintenance.

          Organisation managers can create and delete spaces.
        </p>

        <a href="https://docs.cloud.service.gov.uk/orgs_spaces_users.html#spaces" className="govuk-link">
        ...
        {props.spaces.map(space => (
          <tr key={space.metadata.guid} className="govuk-table__row">
            ...
            <td className="govuk-table__cell">
                ...
                {' '}
                {space.quota ? help.bytesToHuman(space.quota.entity.memory_limit * MEBIBYTE) : 'no limit'}
            </td>
...
export function SpaceTab(props: ISpaceTabProperties): ReactElement {
  return (<>
    <h1 className="govuk-heading-l">
      <span className="govuk-caption-l">Space</span>
      {' '}
    ...
    <div className="govuk-tabs" data-module="govuk-tabs">
      ...
      <ul className="govuk-tabs__list">
        <Tab
          active={props.routePartOf('admin.organizations.spaces.applications.list')}
        ...
        <Tab
          active={props.routePartOf('admin.organizations.spaces.services.list')}
...
export function ApplicationsPage(props: IApplicationPageProperties): ReactElement {
  return (<SpaceTab
      linkTo={props.linkTo}
      organizationGUID={props.organizationGUID}
    ...
    >
    <p className="govuk-body">
      This space contains {props.applications.length} {props.applications.length === 1 ? 'application' : 'applications'}
    </p>

    <table className="govuk-table">
      <thead className="govuk-table__head">
        <tr className="govuk-table__row">
          <th className="govuk-table__header name" scope="col">Application name</th>
          <th className="govuk-table__header" scope="col">Instances</th>
          <th className="govuk-table__header" scope="col">Memory</th>
          <th className="govuk-table__header" scope="col">Disk</th>
          <th className="govuk-table__header" scope="col">Status</th>
          <th className="govuk-table__header" scope="col">URLs</th>
        </tr>
      </thead>
      <tbody className="govuk-table__body">
        {props.applications.map(application => (
          <tr key={application.metadata.guid} className="govuk-table__row">
            <td className="govuk-table__cell">
              <a href={props.linkTo('admin.organizations.spaces.applications.view', {
                organizationGUID: props.organizationGUID,
                spaceGUID: props.space.metadata.guid,
            ...
            <td className="govuk-table__cell" title={`${application.summary.running_instances} running / ${application.summary.instances} desired`}>
              {application.summary.running_instances} / {application.summary.instances}
            </td>
            <td className="govuk-table__cell" title={`Total: ${application.summary.memory * application.summary.instances / KIBIBYTE}GiB`}>
              {application.summary.instances} &times; {help.bytesToHuman(application.summary.memory * MEBIBYTE)}
            </td>
            <td className="govuk-table__cell" title={`Total: ${application.summary.disk_quota * application.summary.instances / KIBIBYTE}GiB`}>
              {application.summary.instances} &times; {help.bytesToHuman(application.summary.disk_quota * MEBIBYTE)}
            </td>
            <td className="govuk-table__cell">
              {application.summary.state.toLowerCase()}
            </td>
            <td className="govuk-table__cell">
              {application.urls.map((url, index) => (<AppLink key={index} href={url} />))}
            </td>
          </tr>
        ))}
      </tbody>
...
export function BackingServicePage(props: IBackingServicePageProperties): ReactElement {
  return (<SpaceTab
    linkTo={props.linkTo}
    organizationGUID={props.organizationGUID}
    routePartOf={props.routePartOf}
  ...
  >
    <p className="govuk-body">
      This space contains {props.services.length} backing {props.services.length === 1 ? 'service' : 'services'}
    </p>

    <table className="govuk-table">
      <thead className="govuk-table__head">
        <tr className="govuk-table__row">
          <th className="govuk-table__header name" scope="col">Service name</th>
          <th className="govuk-table__header" scope="col">Type</th>
          <th className="govuk-table__header" scope="col">Plan</th>
          <th className="govuk-table__header" scope="col">State</th>
        </tr>
      </thead>
      <tbody className="govuk-table__body">
        {props.services.map(service => (
          <tr key={service.metadata.guid} className="govuk-table__row">
            <td className="govuk-table__cell name">
              <a href={props.linkTo('admin.organizations.spaces.services.view', {
                organizationGUID: props.organizationGUID,
                spaceGUID: props.space.metadata.guid,
    ...
    <table className="govuk-table">
      ...
      <tbody className="govuk-table__body">
        {props.events.map(event => (
          <TargetedEventListItem key={event.guid}
            ...
            target={props.actorEmails[event.target.guid] || event.target.name || <code>{event.target.guid}</code>}
