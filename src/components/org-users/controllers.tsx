...
interface ISpaceUsers {
  ...
  readonly users: readonly ISpaceUserRoles[]
}

class ValidationError extends Error {
  public readonly errors: readonly IValidationError[]

  constructor (arrayOfErrors: readonly IValidationError[]) {
...
async function setAllUserRolesForOrg (
  ...
  const orgRoleEndpoints: readonly OrganizationUserRoleEndpoints[] = [
    'billing_managers',
    'managers',
    'auditors'
  ]
...
export async function _getUserRolesByGuid (
  userOrgRoles: readonly IOrganizationUserRoles[],
  spaceUserLists: readonly ISpaceUsers[],
  ...
  for (const user of userOrgRoles) {
    ...
    userRolesByGuid[user.metadata.guid] = {
      ...
      username:
        (accountsUser != null) && accountsUser.username
...
function _excludeUsersWithoutUaaRecord (
  ...
  for (const guid in userRolesByGuid) {
    ...
    if (uaaUsers.some(u => (u != null) && u.id === guid)) {
...
function validatePermissions ({
  org_roles,
  space_roles
}: IUserPostBody): readonly IValidationError[] {
...
function validateEmail ({ email }: IUserPostBody): readonly IValidationError[] {
  const errors: IValidationError[] = []
...
function parseValues (body: IUserPostBody) {
  ...
  return {
    email: body.email,
    org_roles: convert((body.org_roles != null) || {}),
    space_roles: convert((body.space_roles != null) || {})
  }
}
...
export async function inviteUser (
  ...
  try {
    ...
    await setAllUserRolesForOrg(
      ...
      {
        org: (userBody.org_roles != null) || {},
        space: (userBody.space_roles != null) || {}
      }
    )
