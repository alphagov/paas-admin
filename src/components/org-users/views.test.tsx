...
describe(Permission, () => {
  it('should produce a checkbox with appropriate values provided', () => {
    const markup = shallow(<Permission
      checked={false}
      disabled={true}
      name="Permission"
    ...
    expect($('input[type="hidden"][name="permission[current]"]').length).toEqual(1);
    expect($('input[type="hidden"][name="permission[current]"]').prop('value')).toEqual('1');
    expect($('input[type="hidden"][name="permission[desired]"]').length).toEqual(1);
...
describe(PermissionTable, () => {
  const organization = { entity: { name: 'org-name' }, metadata: { guid: 'ORG_GUID' } } as unknown as IOrganization;
  const spaces = [
  ...
  it('should display correctly permissions table element', () => {
    const markup = shallow(<PermissionTable
      billingManagers={2}
      managers={2}
    ...
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][managers]"]:checked').length).toEqual(1);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][managers]"]:disabled').length).toEqual(0);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][billing_managers]"]:checked').length).toEqual(1);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][billing_managers]"]:disabled').length).toEqual(0);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][auditors]"]:checked').length).toEqual(0);

    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_1][managers][desired]"]:checked').length).toEqual(1);
    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_1][developers][desired]"]:checked').length)
      .toEqual(0);
    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_1][auditors][desired]"]:checked').length).toEqual(0);

    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_2][managers][desired]"]:checked').length).toEqual(0);
    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_2][developers][desired]"]:checked').length)
      .toEqual(1);
    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_2][auditors][desired]"]:checked').length).toEqual(0);

    expect($('input[type="checkbox"][name^="space_roles[SPACE_GUID_3][managers][desired]"]:checked').length).toEqual(0);
  ...
  it('should display correctly with disabled checked manager boxes when not enough managers', () => {
    ...
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][managers]"]:checked').length).toEqual(1);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][managers]"]:disabled').length).toEqual(1);
    expect($('input[type="hidden"][name^="org_roles[ORG_GUID][managers][desired]"]').length).toEqual(1);

    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][billing_managers]"]:checked').length).toEqual(1);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][billing_managers]"]:disabled').length).toEqual(1);
  ...
  it('should display correctly with enabled unchecked manager boxes when not enough managers', () => {
    ...
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][managers]"]:checked').length).toEqual(0);
    expect($('input[type="checkbox"][name^="org_roles[ORG_GUID][managers]"]:disabled').length).toEqual(0);
...
describe(DeleteConfirmationPage, () => {
  ...
  it('should ask user to confirm another user deleton', () => {
    const markup = shallow(<DeleteConfirmationPage
      csrf="CSRF_TOKEN"
    ...
    />);
    const $ = cheerio.load(markup.html());
    expect($('h2').text()).toContain(`Are you sure you'd like to remove the following user?`);
    expect($('p').text()).toContain('user-name');
  });
});

describe(OrganizationUsersPage, () => {
  const space = { metadata: { guid: 'SPACE_GUID' }, entity: { name: 'space-name' } } as unknown as ISpace;
  const users = {
    USER_GUID: {
      spaces: [space],
      orgRoles: [],
  ...
  it('should produce the org users view', () => {
    const markup = shallow(<OrganizationUsersPage
      linkTo={route => `__LINKS_TO__${route}`}
      organizationGUID="ORG_GUID"
      privileged={false}
      users={users}
