...
import { IOrganization, IOrganizationUserRoles, ISpace, IUser, OrganizationUserRoles } from '../../lib/cf/types';
...
export function Permission(props: IPermissionProperties): ReactElement {
  return (<>
    <input type="hidden" value={props.state.current ? 1 : 0} name={`${props.namespace}[current]`} />
    {props.disabled ?
      <input type="hidden" value={props.state.desired ? 1 : 0} name={`${props.namespace}[desired]`} /> :
      <></>
    }
      ...
      <div className="govuk-checkboxes">
        <div className="govuk-checkboxes__item">
          <input
            ...
            value="1" />
          <label className="govuk-label govuk-checkboxes__label" htmlFor={props.namespace}>
...
export function PermissionTable(props: IPermissionTableProperties): ReactElement {
  return (<>
    ...
    <table id="roles" className="govuk-table permissions">
      <thead className="govuk-table__head">
        <tr className="govuk-table__row">
          <th className="govuk-table__header name" scope="col">Org name</th>
          <th className="govuk-table__header" scope="col">Org manager</th>
          <th className="govuk-table__header" scope="col">Org billing manager</th>
          <th className="govuk-table__header" scope="col">Org auditor</th>
        </tr>
      </thead>
      <tbody className="govuk-table__body">
        <tr className="govuk-table__row" data-org-guid={props.organization.metadata.guid}>
          <th className="govuk-table__header" scope="row">{props.organization.entity.name}</th>
          <td className="govuk-table__cell ">
            <Permission
              name="Org manager"
              checked={!!props.values.org_roles[props.organization.metadata.guid].managers.desired}
              namespace={`org_roles[${props.organization.metadata.guid}][managers]`}
              state={props.values.org_roles[props.organization.metadata.guid].managers}
              disabled={(props.managers < 2)
                && (props.values.org_roles[props.organization.metadata.guid].managers.current)}
            />
          </td>
          <td className="govuk-table__cell ">
            <Permission
              name="Billing manager"
              checked={!!props.values.org_roles[props.organization.metadata.guid].billing_managers.desired}
              namespace={`org_roles[${props.organization.metadata.guid}][billing_managers]`}
              state={props.values.org_roles[props.organization.metadata.guid].billing_managers}
              disabled={(props.billingManagers < 2)
                && (props.values.org_roles[props.organization.metadata.guid].billing_managers.current)}
            />
          </td>
          <td className="govuk-table__cell ">
            <Permission
              name="Org auditor"
              checked={!!props.values.org_roles[props.organization.metadata.guid].auditors.desired}
              namespace={`org_roles[${props.organization.metadata.guid}][auditors]`}
              state={props.values.org_roles[props.organization.metadata.guid].auditors}
            />
          </td>
        </tr>
      </tbody>
    ...
    <table className="govuk-table permissions">
      <thead className="govuk-table__head">
        <tr className="govuk-table__row">
          <th className="govuk-table__header name" scope="col">Space name</th>
          <th className="govuk-table__header" scope="col">Space manager</th>
          <th className="govuk-table__header" scope="col">Space developer</th>
          <th className="govuk-table__header" scope="col">Space auditor</th>
        </tr>
      </thead>
      <tbody className="govuk-table__body">
        {props.spaces.map(space => (
          <tr key={space.metadata.guid} className="govuk-table__row" data-space-guid={space.metadata.guid}>
            <th className="govuk-table__header" scope="row">{space.entity.name}</th>
            <td className="govuk-table__cell ">
              <Permission
                name="Space manager"
                checked={!!props.values.space_roles[space.metadata.guid].managers.desired}
            ...
            <td className="govuk-table__cell ">
              <Permission
                ...
                checked={!!props.values.space_roles[space.metadata.guid].developers.desired}
                namespace={`space_roles[${space.metadata.guid}][developers]`}
                state={props.values.space_roles[space.metadata.guid].developers}
              />
            </td>
            <td className="govuk-table__cell ">
              <Permission
                name="Space auditor"
                checked={!!props.values.space_roles[space.metadata.guid].auditors.desired}
...
export function DeleteConfirmationPage(props: IDeleteConfirmationPageProperties): ReactElement {
  return (
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-one-half">
        <a href={props.linkTo('admin.organizations.users.edit', {
          organizationGUID: props.organizationGUID,
          userGUID: props.user.metadata.guid,
        })} className="govuk-back-link">See user view</a>
      </div>

      <div className="govuk-grid-column-two-thirds">
        <form method="post" className="govuk-!-mt-r6 paas-remove-user">
          <input type="hidden" name="_csrf" value={props.csrf} />
          <h2 className="govuk-heading-l">Are you sure you'd like to remove the following user?</h2>

          <p className="govuk-heading-m">{props.user.entity.username}</p>

          <button className="govuk-button govuk-button--warning" data-module="govuk-button" data-prevent-double-click>
...
export function SuccessPage(props: ISuccessPageProperties): ReactElement {
  return (
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-one-half">
        <a href={props.linkTo('admin.organizations.users', {
          organizationGUID: props.organizationGUID,
        })} className="govuk-back-link">See all team members</a>
      </div>

      <div className="govuk-grid-column-one-half govuk-!-pt-r1 text-right">
        <a href={props.linkTo('admin.organizations.users.invite', {
          organizationGUID: props.organizationGUID,
        })} role="button" draggable="false" className="govuk-button" data-module="govuk-button">
    ...
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-two-thirds">
        ...
        <h1 className="govuk-heading-l">
          <span className="govuk-caption-l">Team member</span>
          {' '}
          {props.email}
        </h1>
        ...
        <table className="govuk-table">
          <tbody className="govuk-table__body">
            <tr className="govuk-table__row">
              <th scope="row" className="govuk-table__header">Email</th>
        ...
        {!props.isActive ?
          <form method="post" action={props.linkTo('admin.organizations.users.invite.resend', {
            organizationGUID: props.organization.metadata.guid,
            userGUID: props.user.metadata.guid,
        ...
        <a href={props.linkTo('admin.organizations.users.delete', {
          organizationGUID: props.organization.metadata.guid,
          userGUID: props.user.metadata.guid,
        })} className="govuk-link">
      ...
      <div className="govuk-grid-column-two-thirds">
        ...
        <form method="post" className="govuk-!-mt-r6">
          ...
          <div className="govuk-form-group">
            ...
            {props.errors?.filter(error => error.field === 'email').map((error, index) => (
              <span key={index} id="email-error" className="govuk-error-message">
                <span className="govuk-visually-hidden">Error:</span> {error.message}
              </span>
            ))}
            <input
              className={`govuk-input ${props.errors?.some(e => e.field === 'email') ? 'govuk-input--error' : ''}`}
              id="email"
              name="email"
              type="text"
...
export function OrganizationUsersPage(props: IOrganizationUsersPageProperties): ReactElement {
  return (<>
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-two-thirds">
        <h1 className="govuk-heading-l">
      ...
      <div className="govuk-grid-column-one-third text-right">
        {props.privileged ? <a href={props.linkTo('admin.organizations.users.invite', {
          organizationGUID: props.organizationGUID,
        })} role="button" draggable="false" className="govuk-button" data-module="govuk-button">
          Invite a new team member
        </a> : <></>}
    ...
    <table className="govuk-table user-list">
      <thead className="govuk-table__head">
        <tr className="govuk-table__row">
          <th className="govuk-table__header name" scope="col">Email address</th>
          <th className="govuk-table__header authentication" scope="col">
            {props.privileged ? 'Authentication' : ''}
          </th>
          <th className="govuk-table__header is-org-manager" scope="col">Org manager</th>
          <th className="govuk-table__header is-billing-manager" scope="col">Org billing manager</th>
          <th className="govuk-table__header is-org-auditor" scope="col">Org auditor</th>
          <th className="govuk-table__header spaces" scope="col">Spaces assigned or can access</th>
        </tr>
      </thead>
      <tbody className="govuk-table__body">
              ...
              {props.privileged ? (
                props.userOriginMapping[guid] === 'uaa' ?
                  <a href={props.linkTo('admin.organizations.users.edit', {
                    organizationGUID: props.organizationGUID,
                    userGUID: guid,
                  })} className="govuk-link">Password</a> : capitalize(props.userOriginMapping[guid])
              ) : <></>}
            </td>
            <td className="govuk-table__cell">
              {props.users[guid].orgRoles.includes('org_manager') ? <Tick /> : <></>}
            </td>
            <td className="govuk-table__cell">
              {props.users[guid].orgRoles.includes('billing_manager') ? <Tick /> : <></>}
            </td>
            <td className="govuk-table__cell">
              {props.users[guid].orgRoles.includes('org_auditor') ? <Tick /> : <></>}
            </td>
            <td className="govuk-table__cell">
              <ul className="plain">
                {props.users[guid].spaces.map(space => (
                  <li key={space.metadata.guid}>
                    <a href={props.linkTo('admin.organizations.spaces.applications.list', {
                      organizationGUID: props.organizationGUID,
