...
function linker(_route: string, params: any): string {
  return params?.organizationGUID ? `/user/${params.organizationGUID}` : '/test';
}

describe(UserPage, () => {
  const orgA = { metadata: {guid: 'a'}, entity: {name: 'A'} } as unknown as IUserSummaryOrganization;
  const orgB = { metadata: {guid: 'b'}, entity: {name: 'B'} } as unknown as IUserSummaryOrganization;
  const orgC = { metadata: {guid: 'c'}, entity: {name: 'C'} } as unknown as IUserSummaryOrganization;
  const group = { display: 'profile' } as unknown as IUaaGroup;
  const user = { uuid: 'ACCOUNTS-USER-GUID', username: 'jeff0', email: 'jeff@jefferson.com' };
  const logon = new Date(2020, 1, 1);

  it('should display list of organizations', () => {
    const markup = shallow(<UserPage
