...
test('when authenticated', async t => {
  ...
  nock('https://example.com').persist().get('/api/v2/organizations').reply(200, organizations);

  await t.test('should return organisations when accessed root', async t => {
    const response = await agent.get('/');

    t.equal(response.status, 302);
    t.equal(response.header.location, '/organisations');
  });
  ...
  await t.test('should render as text/html with utf-8 charset', async t => {
    const response = await agent.get('/organisations');

    t.equal(response.header['content-type'], 'text/html; charset=utf-8');
  });

  await t.test('should have a Content Security Policy set', async t => {
    const response = await agent.get('/organisations');
  ...
  await t.test('should gzip responses', async t => {
    const response = await agent.get('/organisations');
