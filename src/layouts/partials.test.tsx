...
describe(Header, () => {
  it('should successfully display the header element', () => {
    const markup = shallow(<Header location="London" isPlatformAdmin={false} />);
    expect(markup.find('header nav li a.paas-govuk-tag').text()).toMatch('London');
    ...
    expect(markup.find('header.govuk-header .govuk-header__logotype svg image')
      .filterWhere(item => item.prop('src') === '/assets/images/govuk-logotype-crown.png')).toHaveLength(1);
  });

  it('should show the admin link if platform admin', () => {
    const markup = shallow(<Header location="Ireland" isPlatformAdmin={true} />);
    expect(markup.find('header nav li a.paas-govuk-tag').text()).toMatch('Ireland');
...
describe(Footer, () => {
  it('should successfully display the footer element', async () => {
    ...
    expect(markup.find('.govuk-footer__licence-description a')
      .containsMatchingElement(<a href="https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/">
